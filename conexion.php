...
$password="";//"2587744";
$database="kennetasd";//"lqimport_demo";
$connect=mysqli_connect("localhost",$username,$password,$database);
mysqli_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'", $connect);
...
if($_POST['service']){
    ...
    $sql = "SELECT * FROM avs_employee LEFT JOIN avs_service ON avs_service.ser_id=avs_employee.emp_service WHERE avs_service.ser_id=$service";	
	$data = mysqli_query($connect,$sql);
	 	if ($data->num_rows > 0) {
    while($row = $data->fetch_assoc()) {

	 	$html.='<div class="col-md-6 empb_tarjeta">
                        <div class="empb_contenedor">
                            <div class="row">
                                <div class="col-md-4">
                                    <img class="empb_imagen_perfil" src="img/34229840_Ux6ibJuHtJLGjZmVSfMTzUbtJP28jh00.jpg" alt="">
                                </div>
                                <div class="col-md-8">
                                    <h2>'.$row['emp_name'].'</h2>
                                    <h4>Necesito Niñera cama a dentro</h4>
                                    <h5>Lugar: Miraflores</h5>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    <p>Necesito niñera cama adentro que sea amable con los animales en miraflores, Atenta ...</p>
                                </div>
                                <div class="col-md-12">
                                    <div class="empb_btn">
                                        <a class="btn btn-danger" href="avs_emp_jef_detalle.php">Ver mas detalles</a>
                                    </div>
                                </div>                                
                            </div>
                        </div>
	...
	if($_GET['service']){
		$html ='<option disabled selected="selected">Choose a service</option>';
		$sql = "SELECT * FROM avs_service";	
	 $data = mysqli_query($connect,$sql);
	 if ($data->num_rows > 0) {
    while($row = $data->fetch_assoc()) {
	 	$html.='<option value="'.$row['ser_id'].'">'.$row['ser_name'].'</option>';
	 }
	 echo  $html;
	}
	}
	if($_POST['filter']){
		 ...
		 if($_POST['gender']!='null'){
			$where.="emp_gender='".$_POST['gender']."'";
		}
		if($_POST['language']!='null'){
		    	if($where!=''){$where.=' AND '; }
			$where.="emp_language='".$_POST['language']."'";
		}
	 $today = date('Y-m-d');
     $age = explode("-",$_POST['providerAge']);
     $minAge = $age[1];
     $maxAge = $age[0];
     $date = strtotime(''.$today.'-'.$minAge.' year');	
     $mindate =  date('Y-m-d', $date);
     $date_ = strtotime(''.$today.'-'.$maxAge.' year');	
     $maxdate =  date('Y-m-d', $date_);

     if($_POST['providerAge']!='null'){
     	if($where!=''){$where.=' AND '; }	
     		$where.="emp_birthday>='".$mindate."' AND ";
			$where.="emp_birthday<='".$maxdate."'";
     		
		}
	  $sql = "SELECT * FROM avs_employee WHERE ".$where."";
	$data = mysqli_query($connect,$sql);
	 	if ($data->num_rows > 0){
	 		while($row = $data->fetch_assoc()) {
	 			$html.='<div class="col-md-6 empb_tarjeta">
    ...
    while($row = $data->fetch_assoc()) {
	 ...
	 echo  $html;
		...
		if($_GET['avsAuxiliar']){
		$html ='<option disabled selected="selected">Choose </option>';
		$sql = "SELECT * FROM avs_auxiliar WHERE aux_tip='MK'";	
	 $data = mysqli_query($connect,$sql);
	 if ($data->num_rows > 0) {
    while($row = $data->fetch_assoc()) {
	 	$html.='<option value="'.$row['id_aux'].'">'.$row['aux_nom'].'</option>';
	 }
	 echo  $html;
	}
	}
